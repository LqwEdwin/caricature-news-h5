		...
		methods: {
			...
			update() {
				...
				plus.runtime.getProperty(plus.runtime.appid, function(inf) {
					...
					getApp().globalData.version = inf.version
			...
			getUpdateInfo() {
				...
				var param = "deviceid=" + getApp().globalData.deviceid + "&facility=" + getApp().globalData.facility + "&timestamp=" + time + "&version=" + vm.version
				var sign = w_md5.hex_md5_32Upper(param + this.app_key)
				//向后台发起请求，获取最新版本号
				uni.request({
					url: getApp().globalData.websiteUrl + "/index.php/appv1/init/update?" + param + "&sign=" + sign, // 应用更新接口
					success: (res) => {
						console.log("+++++++++++++++++++++++++++++++++++++",res)
						// let version = res.data.match(/<p class=\"version\" hidden=\"hidden\">(.*)<\/p>/)
						// let content = res.data.match(/<p class=\"content\" hidden=\"hidden\">(.*)<\/p>/)
						// let downurl = res.data.match(/<p class=\"downurl\" hidden=\"hidden\">(.*)<\/p>/)
						// 这里的返回的数据跟后台约定
						let data = [{
							os: getApp().globalData.facility,
							version: res.data.version, //最新版本
							note: res.data.text,
							download_url: res.data.downurl, //下载地址 (微信测试)
							must: res.data.force, // 是否强制更新
						}];
