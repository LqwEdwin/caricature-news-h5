...
export const statusLabel = statusOptions.reduce((acc, cur) => {
...
export const payTypeLabel = payTypeOptions.reduce((acc, cur) => {
	acc[cur.key] = cur.label
	return acc
}, {})
const msgOptions = [{
...
export const msgLabel = msgOptions.reduce((acc, cur) => {
	acc[cur.key] = cur.label
	return acc
}, {})
...
export function getzoom(oneLon, oneLat, twoLon, twoLat) {
	let PI = 3.14159265358979323; //圆周率
	let R = 6371229; //地球半径

	var lon1 = parseFloat(oneLon);
	var lat1 = parseFloat(oneLat);

	var lon2 = parseFloat(twoLon);
	var lat2 = parseFloat(twoLat);
	...
	return (distance > 1000 ? 3 : distance > 500 ? 4 : distance > 200 ? 5 : distance > 50 ? 6 : distance >
		1 ? 11 : 15)
}